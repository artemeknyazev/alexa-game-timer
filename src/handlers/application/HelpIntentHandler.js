...
const GET_ALL_PLAYERS_TOTAL_TIME_HELP_MESSAGE = `Say ${emph('all time')}, or ${emph('how much played')}, to get the total time played per player`
const GET_CURRENT_TURN_TIME_HELP_MESSAGE = `Say ${emph('whose turn')}, to get a current player and a current turn time`
const GET_PLAYER_TOTAL_TIME = `Say player's color and then ${emphr('player time')}, like ${emph('red player time')}, to get the total time played for the player`
...
const HelpIntentHandler = {
  ...
  async handle(handlerInput) {
    ...
    if (state === APP_STATE_NEW_GAME) {
      ...
      speechTexts.push(GET_PLAYER_TOTAL_TIME)
